#### HCBarCategory
# A simple highcharter barplot colored by category.

HCBarCategory <- function(df, dark_mode_on) {
  if (nrow(df) == 0) {
    return(
      shiny::validate(
  ...
  res <- hc_add_series(res, data = df, hcaes(x = category, y = amount, group = category), type = "column")
  if (dark_mode_on) {
